...
foods.forEach((food) => {
  food.calories = (food.carbs * 4) + (food.protein*4) + (food.fat * 2);
})
...
let orderTotal = blueProducts.reduce((total, item) => total+=item.price, 0);
...
let person = {
  ...
  favorites: {color: 'red', number: 7, book: "bob"},
  kids:[{name: 'angel', age: 5}, {name: "cid", age: 12}]
};
